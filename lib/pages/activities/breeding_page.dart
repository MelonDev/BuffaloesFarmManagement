...
class _BreedingPageState extends State<BreedingPage> {
  ...
  String? buffInseminationProviderValue, buffSpeciesValue;
  ...
  List<Widget> listWidget(BuildContext context) {
    return [
      ...
      if (insemination == 1) const SizedBox(height: 14),
      if (insemination == 1)
        textField(
      ...
      if (insemination == 1 && buffInseminationProviderValue == "อื่น ๆ")
        textField(hint: "ระบุ", controller: tfProvider),
      if (insemination == 1) const SizedBox(height: 12),
      const SizedBox(height: 20),
      textHeader(title: "รายละเอียดน้ำเชื้อพ่อพันธุ์"),
      const SizedBox(height: 8),
      textField(hint: "ชื่อ", controller: tfName, required: false),
      const SizedBox(height: 14),
      textField(hint: "เบอร์หู", controller: tfName, required: false),
      if (insemination == 1) ...[
        const SizedBox(height: 20),
